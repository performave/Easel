...
export const queryKeys = {
    ...
    moduleItems: (courseId: number, moduleId: number) =>
        ['moduleItems', courseId, moduleId] as const,
    announcements: (contextCodes: string[]) =>
        ['announcements', ...contextCodes] as const,
    assignmentGroups: (courseId: number) =>
        ['assignmentGroups', courseId] as const,
    ...
    folders: (courseId: number, folderId: number) =>
        ['folders', courseId, folderId] as const,
    files: (courseId: number, folderId: number) =>
        ['files', courseId, folderId] as const,
}
...
export const calendarEventsQueryOptions = (
    ...
    queryOptions({
        ...
        queryFn: () =>
            canvas.calendarEvents(start, end, courseIds.map(contextCode), type),
        staleTime: 2 * 60 * 1000,
        enabled: courseIds.length > 0,
    })
