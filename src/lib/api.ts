...
export const api = {
    ...
    beginLogin: (domain: string) =>
        invoke<BootstrapInfo>('begin_login', { domain }),
    ...
    getAll: <T = unknown>(path: string) =>
        invoke<T[]>('canvas_get_all', { path }),
    request: <T = unknown>(args: RequestArgs) =>
        invoke<T>('canvas_request', args),
...
export const canvas = {
    course: (id: number) =>
        api.get<Course>(
            `/api/v1/courses/${id}?include[]=term&include[]=syllabus_body&include[]=total_scores`
        ),
    courses: () =>
        api.getAll<Course>(
            '/api/v1/courses?enrollment_state=active&include[]=term&include[]=total_scores&per_page=100'
        ),
    ...
    rootFolder: (courseId: number) =>
        api.get<Folder>(`/api/v1/courses/${courseId}/folders/root`),
    calendarEvents: (
        start: string,
        end: string,
        contextCodes: string[],
        ...
        const qs = [
            ...
            `type=${type}`,
            ...contextCodes.map(
                c => `context_codes[]=${encodeURIComponent(c)}`
            ),
            'per_page=100',
        ].join('&')
    ...
    conversations: (
        scope: 'inbox' | 'unread' | 'starred' | 'sent' | 'archived' = 'inbox'
    ) =>
        api.getAll<Conversation>(
            `/api/v1/conversations?scope=${scope}&per_page=50`
        ),
    conversation: (id: number) =>
        api.get<ConversationDetail>(`/api/v1/conversations/${id}`),
    enrollments: (courseId: number) =>
        api.getAll<Enrollment>(
            `/api/v1/courses/${courseId}/enrollments?include[]=avatar_url&per_page=100`
        ),
    tabs: (courseId: number) =>
        api.getAll<CanvasTab>(`/api/v1/courses/${courseId}/tabs`),
    frontPage: (courseId: number) =>
        api.get<CoursePage>(`/api/v1/courses/${courseId}/front_page`),
    ...
    upcomingEvents: () =>
        api.getAll<CalendarEvent>('/api/v1/users/self/upcoming_events'),
    courseNicknames: () =>
        api.get<Record<string, string>>('/api/v1/users/self/course_nicknames'),
    ...
    dashboardPositions: (userId: number) =>
        api.get<{ dashboard_positions: Record<string, number> }>(
            `/api/v1/users/${userId}/dashboard_positions`
        ),
    ...
    colors: (userId: number) =>
        api.get<{ custom_colors: Record<string, string> }>(
            `/api/v1/users/${userId}/colors`
        ),
