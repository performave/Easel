...
export function parseCourseContextCode(code: string | null | undefined): number | null {
...
export function parseNicknames(raw: Record<string, string>): Record<number, string> {
...
export function parsePositionsOrder(positions: Record<string, number>): number[] {
...
export function parseColors(colors: Record<string, string>): Record<number, string> {
  const out: Record<number, string> = {};
  for (const [asset, color] of Object.entries(colors)) {
    const id = parseCourseContextCode(asset);
    if (id != null && color) out[id] = color.startsWith("#") ? color : `#${color}`;
  }
  return out;
}
