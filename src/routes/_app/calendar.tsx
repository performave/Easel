...
function CalendarPage() {
    ...
    const eventsQuery = useQuery(
        calendarEventsQueryOptions(start, end, courseIds, 'event')
    )
    const assignmentsQuery = useQuery(
        calendarEventsQueryOptions(start, end, courseIds, 'assignment')
    )
    ...
    const loading =
        coursesPending ||
        (courseIds.length > 0 &&
            (eventsQuery.isPending || assignmentsQuery.isPending))
                    ...
                    <CardContent className='p-0'>
                        <div className='grid grid-cols-7 border-b'>
                            {[
                                ...
                                <div
                                    key={d}
                                    className='text-muted-foreground border-r p-2 text-xs font-medium last:border-r-0'
                                >
                                    {d}
                                </div>
                            ))}
                                            ...
                                            {dayEvents.slice(0, 3).map(ev => {
                                                ...
                                                return (
                                                    <a
                                                        ...
                                                        target='_blank'
                                                        rel='noreferrer'
                                                        className='bg-accent hover:bg-accent/80 block truncate rounded border-l-2 px-1.5 py-0.5 text-[11px]'
                                                        style={
                                                            color
                                                                ? {
                                                                      borderLeftColor:
