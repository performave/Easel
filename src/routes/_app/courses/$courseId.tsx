...
function CourseLayout() {
    ...
    const visibleTabs = tabs.filter(
        t => t.id === 'home' || !userHidden.includes(t.id)
    )
        ...
        <div className='flex h-full w-full min-w-0 flex-col'>
          ...
          the scrollbar never clips the tab bar below it. */}
            <div
                ref={titleRef}
                className={cn(
                    'bg-background shrink-0 overflow-hidden transition-[max-height,opacity,padding] duration-200 ease-in-out',
                    titleCollapsed
                        ? 'max-h-0 pt-0 pb-0 opacity-0'
                        : 'max-h-40 pt-6 pb-4 opacity-100'
                )}
            >
                    ...
                    {canvasTabs && allSupportedTabs.length > 1 && (
                        <Popover>
                            ...
                            <PopoverContent className='w-56 p-2' align='end'>
                                <p className='text-muted-foreground px-3 py-1.5 text-xs font-semibold'>
                                    Customize tabs
                                </p>
