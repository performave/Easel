...
export const Route = createFileRoute('/_app/courses/$courseId/files')({
    loader: ({ context, params }) => {
        void context.queryClient.prefetchQuery(
            rootFolderQueryOptions(Number(params.courseId))
        )
    },
    component: FilesPage,
})
...
function FilesPage() {
    ...
    const { data: rootFolder, isError: isRootError } = useQuery(
        rootFolderQueryOptions(id)
    )
    ...
    const foldersOptions = useMemo(
        () =>
            currentFolderId == null
                ? null
                : foldersQueryOptions(id, currentFolderId),
        [id, currentFolderId]
    )
    const filesOptions = useMemo(
        () =>
            currentFolderId == null
                ? null
                : filesQueryOptions(id, currentFolderId),
        [id, currentFolderId]
    )
    const foldersQuery = useQuery({
        ...(foldersOptions ?? foldersQueryOptions(id, -1)),
        enabled: currentFolderId != null,
    })
    const filesQuery = useQuery({
        ...(filesOptions ?? filesQueryOptions(id, -1)),
        enabled: currentFolderId != null,
    })
    const folders = foldersQuery.data ?? []
    const files = filesQuery.data ?? []
    const isRestricted =
        isRootError || foldersQuery.isError || filesQuery.isError
            ...
            <div className='divide-y overflow-hidden rounded-md border'>
                {(foldersQuery.isPending || filesQuery.isPending) && (
                    <SkeletonList
                        ...
                        wrapperClassName='space-y-1 p-3'
                    />
                )}
                {folders?.map(f => (
                    <button
