...
import { IconBook2, IconChartBar, IconChecklist, IconMail } from "@tabler/icons-react";
...
export function StatsRow({ courses }: { courses: Course[] }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
      <Stat icon={<IconBook2 />} label="Active courses" value={courses.length} />
      <Stat icon={<IconChartBar />} label="Average grade" value={avg != null ? `${avg.toFixed(1)}%` : "—"} />
      <Stat icon={<IconChecklist />} label="To-do items" value={todoQuery.data?.length ?? "—"} />
      <Stat icon={<IconMail />} label="Unread messages" value={unreadQuery.data?.length ?? "—"} />
    </div>
  );
}
