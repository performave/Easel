...
export function useRemotePrefsSync(): number | null {
    ...
    const positionsQuery = useQuery({
        ...dashboardPositionsQueryOptions(userId ?? 0),
        enabled: userId != null,
    })
    const colorsQuery = useQuery({
        ...colorsQueryOptions(userId ?? 0),
        enabled: userId != null,
    })

    useEffect(() => {
        hydrateRemote({
            order: positionsQuery.data
                ? parsePositionsOrder(positionsQuery.data.dashboard_positions)
                : undefined,
            nicknames: nicknamesQuery.data
                ? parseNicknames(nicknamesQuery.data)
                : undefined,
            colors: colorsQuery.data
                ? parseColors(colorsQuery.data.custom_colors)
                : undefined,
        })
    }, [
        hydrateRemote,
